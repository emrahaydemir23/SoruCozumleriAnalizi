...
try:
    ...
    df2 = ps.sqldf(f"Select [Ders Adı],max([Zaman damgası]) as [Zaman],sum([Çözüm Süresi])*1.0/sum([Toplam Sayı]) as [Ortalama Çözüm Süresi], count([Zaman damgası]) as [Çözüm Sayısı],sum([Toplam Sayı]) as [Bugüne Kadar Toplam Soru Sayısı] from df group by [Ders Adı]")
    ...
    df4 = ps.sqldf("select [Ders Adı], sum([Doğru Sayısı]) as [Doğru], sum([Yanlış Sayısı]) as [Yanlış], sum([Boş Sayısı]) as [Boş], sum([Toplam Sayı]) as [Toplam], (sum([Doğru Sayısı])-(sum([Yanlış Sayısı])/3)) as [Net],(sum([Doğru Sayısı])-(sum([Yanlış Sayısı])/3))*100.0/(sum([Toplam Sayı])) as [Net Oranı],sum([Çözüm Süresi]) as [Toplam Süre],sum([Çözüm Süresi])*1.0/sum([Toplam Sayı]) as [Ortalama Süre],sum([Çözüm Süresi])*100.0/{genelToplamSure} as [Toplam Zaman Oranı] from df group by [Ders Adı]")
