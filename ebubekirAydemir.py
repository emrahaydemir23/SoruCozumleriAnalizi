...
df2 = ps.sqldf("Select [Ders Adı],max([Zaman damgası]) as [Zaman],avg([Çözüm Süresi]) as [Ortalama Çözüm Süresi], count([Zaman damgası]) as [Çözüm Sayısı] from df group by [Ders Adı]")
...
df3 = ps.sqldf("Select [Kitap Adı],max([Zaman damgası]) as [Zaman],avg([Çözüm Süresi]) as [Ortalama Çözüm Süresi], count([Zaman damgası]) as [Çözüm Sayısı] from df group by [Kitap Adı]")
